...
loam_sdk::import_contract!(soroban_deployer_test_contract);
...
fn test_deploy_from_contract() {
    ...
    let wasm_hash = env.deployer().upload_contract_wasm(soroban_deployer_test_contract::WASM);
    ...
    let client = soroban_deployer_test_contract::Client::new(&env, &contract_id);
